...
section('پیامکِ خوش‌آمدگویی (۱۷.۱ بخش ۴)');
{
  ...
  ok('🔒 ردیفِ خوش‌آمدگویی فقط برای حسابِ شماره‌دار',
     /\$\{verified \? `<div class="kv"><span>📨 خوش‌آمدگویی<\/span>/.test(src));
...
section('پروفایل مهمان و صفحهٔ ورود — پاک‌سازیِ متن‌ها');
{
  ...
  ok('و برچسبِ نقش درست خوانده می‌شود',
     src.includes("'👤 کاربر'") && src.includes("'👑 مدیر'"),
     (src.match(/#cRole'\)\.textContent[^\n]*/) || [''])[0]);
  ...
  ok('مهمان کارتِ «📱 اتصال به حساب» دارد', src.includes('📱 اتصال به حساب'));
